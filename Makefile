...
#   MKDIRS, if not `no', will build# ect directories at
...
#   do-top-:      creates the top level# ect directory.
#   do-tools-:    creates# ect directories for the host toolchain.
#   do-tools:        builds host toolchain.
#   params:          record the values of variables that might affect the
#                    build.
#  # :             creates# ect directories.
...
#
# If _SRC_TOP__ gets set here, we will end up with a directory that may
# not be the top level# dir, because "make# " can happen in the *middle*
# of "make build" (long after <bsd.own.mk> is calculated it).  So, pre-set
# _SRC_TOP__ here so it will not be added to ${.MAKEOVERRIDES}.
#
...
# SRCTAR=src.tar.gz
# do-installsrc:
# 	cd ${.CURDIR} && git ls-tree -r HEAD --name-only | tar czf ${.DIR}/$(SRCTAR) -T -
# 	${INSTALL_FILE} ${.DIR}/$(SRCTAR) ${DESTDIR}/usr/src/
# .endif # defined(__MINIX)
